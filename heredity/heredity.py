...
def joint_probability(people, one_gene, two_genes, have_trait):
    ...
    for p in people.keys():
        ...
        father = detail.get("father")
        trait = False
        if p in have_trait:
            trait = True

        if p in one_gene:
            prob *= PROBS["trait"][1][trait]
            if mother is None:
                prob *= PROBS["gene"][1]
            else:
...
def update(probabilities, one_gene, two_genes, have_trait, p):
    ...
    for key in probabilities.keys():
        ...
        if key in have_trait:
            trait = True
        probabilities[key]["trait"][trait] += p
        if key in one_gene:
...
def normalize(probabilities):
    ...
    for p in probabilities.keys():
        count = 0
        trait_values = probabilities[p]["trait"]
        for values in trait_values:
            count += probabilities[p]["trait"][values]
        count = 1 / count

        for values in trait_values:
            probabilities[p]["trait"][values] *= count

        count = 0
        gene_values = probabilities[p]["gene"]
        for values in gene_values:
            count += probabilities[p]["gene"][values]
        count = 1 / count

        for values in gene_values:
            probabilities[p]["gene"][values] *= count
